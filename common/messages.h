...
// If the uint16_t tag of a message is to generated by some sender sequentially,
// use these value.
// values 1..max-1 establish unique IDs in the request/response message streams.
enum class MsgTag : uint16_t {
    none = 0,
    max = 9999,
    next = 0xffff // transmitter autogenerates value
};
...
public:
    ...
    MsgTag	tag ; // numeric prefix of paresed line
    ...
    bool is(int argIdx, const char* val) const ;
    // split buffer text line into token, buffer is modifed and must stay persistent
    void tokenize(const char* line) ;
    char* asString(char* result, int resultSize, int startTokenIndex) const ;

    uint8_t parseUniBusCycleArg0() const ;
} ;
...
public:
    ...
    static void buildSyntaxError(const TokenList *tokenList, const char* syntax, const char* msg);
    ...
    virtual const char* initFromArgString(const TokenList *tokenList, int startTokenIndex) ;
    virtual const char* initFromArgToken(const TokenList *tokenList, int startTokenIdx) ;
    ...
    static MsgTag errorTag ; // error is response to a illformed request with that tag
...
//	}
    ...
    MsgTag tag  ; // marker 0..9999

    const char* syntaxInfo = nullptr; // helper text with command syntax

    char *renderFormat(const char* _fmt, ...) ; // helper
...
//        msg[0] = 0; // no message
//    }
    ResponseOK(MsgTag _tag): Message(_tag) {
        msg[0] = 0; // no message
    }
    ResponseOK(MsgTag _tag, const char* _fmt, ...): Message(_tag) {
        va_list argptr;
        va_start(argptr, _fmt);
        vsnprintf(msg, bufflen, _fmt, argptr);
        msg[bufflen - 1] = 0; // terminate on trunc
    }
    const char *initFromArgString(const TokenList *tokenList, int startTokenIndex) override ;

    const char* render() override {
        if (strlen(msg))
            return renderFormat("OK %s", msg);
        else
            return renderFormat("OK");
    }
...
//    }
    ResponseError(MsgTag _tag): Message(_tag) {
        errormsg[0] = 0; // no message
    }

    ResponseError(MsgTag _tag, const char* _fmt, ...): Message(_tag) {
        va_list argptr;
        va_start(argptr, _fmt);
...
public:
    RequestReset(MsgTag _tag): Message(_tag) {}
    const char* render() override {
        return renderFormat("RESET");
    }
...
public:
    RequestUnibusSignalsRead(MsgTag _tag): Message(_tag) {}
    const char* render() override {
        return "R US";
    }
    const char *initFromArgToken(const TokenList *tokenList, int startTokenIdx) override ;
...
public:
    ...
    ResponseUnibusSignals(MsgTag _tag): Message(_tag) {} // all fields default to 0
    ResponseUnibusSignals(MsgTag _tag, UnibusSignals _signals): Message(_tag), signals(_signals) {} // all fields default to 0
    ResponseUnibusSignals(MsgTag _tag, uint32_t _addr, uint16_t _data, uint8_t _c1c0, uint8_t _msyn, uint8_t _ssyn,
                          uint8_t _pbpa, uint8_t _intr, uint8_t _br74, uint8_t _bg74, uint8_t _npr, uint8_t _npg, uint8_t _sack, uint8_t _bbsy,
                          uint8_t _init, uint8_t _aclo, uint8_t _dclo) :
        Message(_tag),
...
public:
    char signalName[10];
    uint32_t val;
    RequestUnibusSignalWrite(MsgTag _tag): Message(_tag) {}
    RequestUnibusSignalWrite(MsgTag _tag, const char* _signalName, uint32_t _val): Message(_tag) {
        strcpy(signalName, _signalName);
        val = _val;
    }
    const char *initFromArgToken(const TokenList *tokenList, int startTokenIdx) override ;
    const char* render() override {
        return renderFormat("W US %s %x", signalName, val);
    }
    void* process() override; // to be defined differently in M93X2 PCB and PC host
};
...
public:
    RequestKY11LBSignalsRead(MsgTag _tag): Message(_tag) {}
    const char *initFromArgToken(const TokenList *tokenList, int startTokenIdx) override ;
    const char* render() override {
        return renderFormat("R Y");
    }
    void* process() override; // to be defined differently in M93X2 PCB and PC host
};
...
public:
    ...
    uint8_t lir = 0;
    uint8_t pbp = 0;
    ResponseKY11LBSignals(MsgTag _tag): Message(_tag) {}
    ResponseKY11LBSignals(MsgTag _tag, uint16_t _mpc, uint8_t _fp11, uint8_t _sbf, uint8_t _lir, uint8_t _pbp) :
        Message(_tag),
        mpc(_mpc), fp11(_fp11), sbf(_sbf), lir(_lir), pbp(_pbp) {}
    const char *initFromArgToken(const TokenList *tokenList, int startTokenIdx) override ;
    const char* render() override {
        return renderFormat("Y %x %d %d %d %d", mpc, fp11, sbf, lir, pbp);
    }
    void* process() override; // to be defined differently in M93X2 PCB and PC host
};
...
public:
    ...
    RequestKY11LBSignalWrite(MsgTag _tag): Message(_tag) {}
    RequestKY11LBSignalWrite(MsgTag _tag, const char* _signalName, uint16_t _val): Message(_tag) {
        strcpy(signalName, _signalName);
        val = _val;
    }
    const char *initFromArgToken(const TokenList *tokenList, int startTokenIdx) override ;
    const char* render() override {
        if (val <= 1)
            return renderFormat("W Y %s %x", signalName, val);
        else
            return renderFormat("W Y %s P", signalName);
    }
    void* process() override; // to be defined differently in M93X2 PCB and PC host
};
...
public:
    char	channel;  // A, B
    RequestKM11SignalsRead(MsgTag _tag): Message(_tag) {}
    RequestKM11SignalsRead(MsgTag _tag, char _channel) : Message(_tag), channel(_channel) {}
    const char *initFromArgToken(const TokenList *tokenList, int startTokenIdx) override ;
    const char* render() override {
        return renderFormat("R M %c", channel);
    }
    void* process() override; // to be defined differently in M93X2 PCB and PC host
};
...
public:
    ...
    ResponseKM11Signals(MsgTag _tag): Message(_tag) {}
    ResponseKM11Signals(MsgTag _tag, char _channel, uint8_t _gpio0a, uint8_t _gpio0b, uint8_t _gpio1a, uint8_t _gpio1b) :
        Message(_tag),
        channel(_channel), gpio0a(_gpio0a), gpio0b(_gpio0b), gpio1a(_gpio1a), gpio1b(_gpio1b) {}
    const char *initFromArgToken(const TokenList *tokenList, int startTokenIdx) override ;
    const char* render() override {
        return renderFormat("M %c %x %x %x %x", channel, gpio0a, gpio0b, gpio1a, gpio1b);
    }
...
public:
    ...
    RequestKM11SignalsWrite(MsgTag _tag): Message(_tag) {}
    RequestKM11SignalsWrite(MsgTag _tag, char _channel, uint8_t _val03) : Message(_tag),
        channel(_channel), val03(_val03) {}
    const char *initFromArgToken(const TokenList *tokenList, int startTokenIdx) override ;
    const char* render() override {
        return renderFormat("W M %c %x", channel, val03);
    }
#if !defined(PLATFORM_ARDUINO)
    bool A1,B2,U1,V2;
...
public:
    ...
    RequestMcp23017RegistersRead(MsgTag _tag): Message(_tag) {}
    RequestMcp23017RegistersRead(MsgTag _tag, uint8_t _group, uint8_t _addr, char _half) :
        Message(_tag),
        group(_group), addr(_addr), half(_half) {}
    const char *initFromArgToken(const TokenList *tokenList, int startTokenIdx) override ;
    const char* render() override {
        return renderFormat("R MCP %d %d %c", group, addr, half);
    }
    void* process() override; // to be defined differently in M93X2 PCB and PC host
};
...
public:
    ...
    ResponseMcp23017Registers(MsgTag _tag): Message(_tag) {}
    ResponseMcp23017Registers(MsgTag _tag, uint8_t _group, uint8_t _addr, char _half,
    ...
    const char* render() override {
        ...
        return renderFormat(fmt, (int)group, (int)addr,
...
public:
    ...
    RequestLedWrite(MsgTag _tag): Message(_tag) {}
    RequestLedWrite(MsgTag _tag, uint8_t _unit, uint32_t _off_period_millis, uint32_t _on_period_millis) :
        Message(_tag),
        unit(_unit), off_period_millis(_off_period_millis), on_period_millis(_on_period_millis) {}
    const char *initFromArgToken(const TokenList *tokenList, int startTokenIdx) override ;
    const char* render() override {
        return renderFormat("W LED %d %lu %lu", unit, (long unsigned) off_period_millis, (long unsigned) on_period_millis);
    }
    void* process() override; // to be defined differently in M93X2 PCB and PC host
};
...
public:
    RequestSwitchesRead(MsgTag _tag): Message(_tag) {}
    const char *initFromArgToken(const TokenList *tokenList, int startTokenIdx) override ;
    const char* render() override {
        return renderFormat("R SW");
    }
    void* process() override; // to be defined differently in M93X2 PCB and PC host
};
...
public:
    ...
    ResponseSwitches(MsgTag _tag): Message(_tag) {}
    ResponseSwitches(MsgTag _tag, uint8_t _val05) : Message(_tag), val05(_val05) {}
    const char *initFromArgToken(const TokenList *tokenList, int startTokenIdx) override ;
    const char* render() override {
        return renderFormat("SW %x", val05);
    }
    void* process() override; // to be defined differently in M93X2 PCB and PC host
};
...
public:
    RequestVersionRead(MsgTag _tag): Message(_tag) {}
    const char *initFromArgToken(const TokenList *tokenList, int startTokenIdx) override ;
    const char* render() override {
        return renderFormat("R VERSION");
    }
    void* process() override; // to be defined differently in M93X2 PCB and PC host
};
...
public:
    ...
    ResponseVersion(MsgTag _tag): Message(_tag) {}
    ResponseVersion(MsgTag _tag, const char* _fmt, ...): Message(_tag) {
        va_list argptr;
        va_start(argptr, _fmt);
    ...
    const char *initFromArgString(const TokenList *tokenList, int startTokenIndex) override ;
    const char* render() override {
        return renderFormat("VERSION %s", version);
    }
    void* process() override; // to be defined differently in M93X2 PCB and PC host
};
...
public:
    uint32_t addr;
    RequestUnibusExam(MsgTag _tag): Message(_tag) {}
    RequestUnibusExam(MsgTag _tag, uint32_t _addr) : Message(_tag), addr(_addr) {}
    const char *initFromArgToken(const TokenList *tokenList, int startTokenIdx) override ;
    const char* render() override {
        return renderFormat("E %lo", (long unsigned) addr);
    }
    void* process() override; // to be defined differently in M93X2 PCB and PC host
};
...
public:
    ...
    ResponseUnibusCycle(MsgTag _tag): Message(_tag) {}
    ResponseUnibusCycle(MsgTag _tag, uint8_t _c1c0, uint32_t _addr, uint16_t _data, bool _nxm, bool _requested) :
        Message(_tag),
    ...
    const char* render() override {
        ...
        if (nxm)
            renderFormat("%s %lo NXM", cycleText[c1c0], (unsigned long) addr);
        else
            renderFormat("%s %lo %o", cycleText[c1c0], (unsigned long) addr, data);
        if (!requested)
...
public:
    uint32_t addr;
    uint16_t data;
    RequestUnibusDeposit(MsgTag _tag): Message(_tag) {}
    RequestUnibusDeposit(MsgTag _tag, uint32_t _addr, uint16_t _data) : Message(_tag), addr(_addr), data(_data) {}
    const char *initFromArgToken(const TokenList *tokenList, int startTokenIdx) override ;
    const char* render() override {
        return renderFormat("D %lo %o", (unsigned long) addr, data);
    }
    void* process() override; // to be defined differently in M93X2 PCB and PC host
};
...
public:
    RequestUnibusSize(MsgTag _tag): Message(_tag) {}
    const char *initFromArgToken(const TokenList *tokenList, int startTokenIdx) override ;
    const char* render() override {
        return renderFormat("UBSIZE");
    }
    void* process() override; // to be defined differently in M93X2 PCB and PC host
};
...
public:
    uint32_t seed;
    RequestUnibusTest(MsgTag _tag): Message(_tag) {}
    RequestUnibusTest(MsgTag _tag, uint32_t _seed) : Message(_tag), seed(_seed)  {}
    const char *initFromArgToken(const TokenList *tokenList, int startTokenIdx) override ;
    const char* render() override {
        return renderFormat("UBTEST %x", seed);
    }
    void* process() override; // to be defined differently in M93X2 PCB and PC host
};
...
public:
    uint16_t fileNr ;
    RequestBoot(MsgTag _tag): Message(_tag) {}
    RequestBoot(MsgTag _tag, uint16_t _fileNr) : Message(_tag), fileNr(_fileNr) {}
    const char *initFromArgToken(const TokenList *tokenList, int startTokenIdx) override ;
    const char* render() override {
        return renderFormat("BOOT %ld", (unsigned long) fileNr);
    }
    void* process() override; // to be defined differently in M93X2 PCB and PC host
};
...
public:
    RequestRegDef(MsgTag _tag): Message(_tag) {}
    const char *initFromArgToken(const TokenList *tokenList, int startTokenIdx) override ;
    const char* render() override {
        return renderFormat("R REGDEF");
    }
    void* process() override; // to be defined differently in M93X2 PCB and PC host
};
...
public:
    ResponseRegDef(MsgTag _tag): Message(_tag) {}
...
public:
    RequestRegVal(MsgTag _tag): Message(_tag) {}
    const char *initFromArgToken(const TokenList *tokenList, int startTokenIdx) override ;
    const char* render() override {
        return renderFormat("R REGVAL");
    }
    void* process() override; // to be defined differently in M93X2 PCB and PC host
};
...
public:
    ResponseRegVals(MsgTag _tag): Message(_tag) {}
    std::vector<Variable> registers;
    const char *initFromArgToken(const TokenList *tokenList, int startTokenIdx) override ;
    const char* render() override {
        renderFormat("REGVAL");
