...
uint64_t getBitSubField(uint64_t value, unsigned bitFrom, unsigned bitTo)  {
    ...
    for (bitDestIdx=0, bitSrcIdx = bitFrom ; bitSrcIdx <= bitTo ; bitSrcIdx++, bitDestIdx++)
        if (value & ((uint64_t)1 << bitSrcIdx))
            result |= ((uint64_t)1 << bitDestIdx) ;
    return result ;
}

bool getBit(uint64_t value, unsigned bitIdx) {
		return !!getBitSubField(value, bitIdx, bitIdx);
}
...
uint64_t BinaryString::getValueFromString() {
    ...
    for (char& c : text) {
        ...
        if (c == '0' || c == '1') {
            if (msbFirst)
                result |= ( (uint64_t)1 << (bitCount-bitIdx-1)) ;
            else result |= ( (uint64_t)1 << bitIdx) ;
            bitIdx++ ;
        }
    }
    return result ;
